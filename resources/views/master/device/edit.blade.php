<x-app-layout>
    @include('layouts.partials.page-header', [
        'title' => 'Edit Device Mesin',
        'subtitle' => 'Perbarui data mesin Fingerspot yang sudah ada.'
    ])

    @include('layouts.partials.flash-message')

    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('master.device.update', $device) }}" method="POST" class="grid grid-cols-1 gap-6 md:grid-cols-2">
            @csrf
            @method('PUT')

            <div class="md:col-span-2">
                <label class="mb-2 block text-sm font-semibold text-slate-700">Nama Device</label>
                <input type="text" name="nama" value="{{ old('nama', $device->nama) }}"
            ...
            <div>
                ...
                <input type="text" name="cloud_id" value="{{ old('cloud_id', $device->cloud_id) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div>
                ...
                <select name="timezone"
                    ...
                    @foreach ($timezones as $timezone)
                        <option value="{{ $timezone }}" @selected(old('timezone', $device->timezone) === $timezone)>
                            {{ $timezone }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="datetime-local" name="last_seen_at"
                    value="{{ old('last_seen_at', $device->last_seen_at ? $device->last_seen_at->format('Y-m-d\TH:i') : '') }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div class="flex items-center gap-3 pt-8">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" id="is_active" name="is_active" value="1" @checked(old('is_active', $device->is_active))
