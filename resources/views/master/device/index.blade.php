<x-app-layout>
    @include('layouts.partials.page-header', [
        'title' => 'Device Mesin',
        'subtitle' =>
            'Kelola mesin Fingerspot, lokasi perangkat, serial number, dan status aktif device untuk kebutuhan multi-device attendance.',
        'actions' =>
            '<a href="' .
            route('master.device.create') .
            '" class="inline-flex items-center rounded-2xl bg-white px-4 py-2 text-sm font-semibold text-blue-700 shadow-sm ring-1 ring-white/70 hover:bg-blue-50">+ Tambah Device</a>',
    ])
    ...
    <div class="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="GET" action="{{ route('master.device.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-5">
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('master.device.index') }}"
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">
                                Timezone</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">IP
                                Address</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">
                                Last Seen</th>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">
                                        ...
                                        <form action="{{ route('master.device.get-attlog', $item) }}" method="POST"
                                            ...
                                            <div class="flex flex-wrap items-center gap-2">
                                                <input type="date" name="start_date"
                                                    value="{{ now()->timezone('Asia/Jakarta')->toDateString() }}"
                                                    class="w-[140px] rounded-xl border border-slate-300 px-3 py-2 text-xs text-slate-700">
                                            ...
                                            <button type="submit"
                                                onclick='return confirm("Ambil data attlog dari device ini? Maksimal range 2 hari per request.")'
                                                class="inline-flex rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700 hover:bg-amber-100">
                                                Get Attlog
                                            </button>
                                        </form>

                                        <form action="{{ route('master.device.destroy', $item) }}" method="POST"
