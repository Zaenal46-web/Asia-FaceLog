<x-app-layout>
    @include('layouts.partials.page-header', [
        'title' => 'Edit Rule Shift per Kategori',
        'subtitle' => 'Perbarui rule shift kategori yang sudah ada.'
    ])

    @include('layouts.partials.flash-message')

    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('master.kategori-shift.update', $kategoriShift) }}" method="POST" class="grid grid-cols-1 gap-6 md:grid-cols-2">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="kategori_karyawan_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
                    ...
                    @foreach ($kategoriOptions as $kategori)
                        <option value="{{ $kategori->id }}" @selected(old('kategori_karyawan_id', $kategoriShift->kategori_karyawan_id) == $kategori->id)>
            ...
            <div>
                ...
                <select name="shift_master_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
                    ...
                    @foreach ($shiftOptions as $shift)
                        <option value="{{ $shift->id }}" @selected(old('shift_master_id', $kategoriShift->shift_master_id) == $shift->id)>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="nama_rule" value="{{ old('nama_rule', $kategoriShift->nama_rule) }}"
            ...
            <div>
                ...
                <input type="number" name="prioritas" value="{{ old('prioritas', $kategoriShift->prioritas) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div class="flex items-center gap-3 pt-8">
                <input type="hidden" name="is_default" value="0">
                <input type="checkbox" id="is_default" name="is_default" value="1" @checked(old('is_default', $kategoriShift->is_default))
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="checkbox" id="lintas_hari" name="lintas_hari" value="1" @checked(old('lintas_hari', $kategoriShift->lintas_hari))
                    class="h-5 w-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="number" name="window_masuk_mulai_menit" value="{{ old('window_masuk_mulai_menit', $kategoriShift->window_masuk_mulai_menit) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Window Masuk Selesai (menit)</label>
                <input type="number" name="window_masuk_selesai_menit" value="{{ old('window_masuk_selesai_menit', $kategoriShift->window_masuk_selesai_menit) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Window Pulang Mulai (menit)</label>
                <input type="number" name="window_pulang_mulai_menit" value="{{ old('window_pulang_mulai_menit', $kategoriShift->window_pulang_mulai_menit) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Window Pulang Selesai (menit)</label>
                <input type="number" name="window_pulang_selesai_menit" value="{{ old('window_pulang_selesai_menit', $kategoriShift->window_pulang_selesai_menit) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Toleransi Telat (menit)</label>
                <input type="number" name="toleransi_telat_menit" value="{{ old('toleransi_telat_menit', $kategoriShift->toleransi_telat_menit) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Toleransi Pulang Cepat (menit)</label>
                <input type="number" name="toleransi_pulang_cepat_menit" value="{{ old('toleransi_pulang_cepat_menit', $kategoriShift->toleransi_pulang_cepat_menit) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Toleransi Lembur (menit)</label>
                <input type="number" name="toleransi_lembur_menit" value="{{ old('toleransi_lembur_menit', $kategoriShift->toleransi_lembur_menit) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Menit Istirahat Default</label>
                <input type="number" name="menit_istirahat_default" value="{{ old('menit_istirahat_default', $kategoriShift->menit_istirahat_default) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div class="space-y-4 md:col-span-2">
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" id="istirahat_aktif" name="istirahat_aktif" value="1" @checked(old('istirahat_aktif', $kategoriShift->istirahat_aktif))
                        class="h-5 w-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" id="istirahat_otomatis_potong" name="istirahat_otomatis_potong" value="1" @checked(old('istirahat_otomatis_potong', $kategoriShift->istirahat_otomatis_potong))
                        class="h-5 w-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" id="is_active" name="is_active" value="1" @checked(old('is_active', $kategoriShift->is_active))
                        class="h-5 w-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500">
