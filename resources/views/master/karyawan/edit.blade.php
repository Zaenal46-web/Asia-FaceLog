<x-app-layout>
    @include('layouts.partials.page-header', [
        'title' => 'Edit Karyawan',
        'subtitle' => 'Perbarui data karyawan yang sudah ada.'
    ])

    @include('layouts.partials.flash-message')

    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('master.karyawan.update', $karyawan) }}" method="POST" class="grid grid-cols-1 gap-6 md:grid-cols-2">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="pin_fingerspot" value="{{ old('pin_fingerspot', $karyawan->pin_fingerspot) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div>
                ...
                <select name="kategori_karyawan_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
                    ...
                    @foreach ($kategoriOptions as $kategori)
                        <option value="{{ $kategori->id }}" @selected(old('kategori_karyawan_id', $karyawan->kategori_karyawan_id) == $kategori->id)>
            ...
            <div>
                ...
                <select name="device_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
                    ...
                    @foreach ($deviceOptions as $device)
                        <option value="{{ $device->id }}" @selected(old('device_id', $karyawan->device_id) == $device->id)>
                            {{ $device->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="tanggal_masuk" value="{{ old('tanggal_masuk', $karyawan->tanggal_masuk ? $karyawan->tanggal_masuk->format('Y-m-d') : '') }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Status Kerja</label>
                <input type="text" name="status_kerja" value="{{ old('status_kerja', $karyawan->status_kerja) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div class="flex items-center gap-3 pt-8">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" id="is_active" name="is_active" value="1" @checked(old('is_active', $karyawan->is_active))
