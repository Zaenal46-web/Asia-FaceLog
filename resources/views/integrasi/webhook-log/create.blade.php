<x-app-layout>
    @include('layouts.partials.page-header', [
        'title' => 'Tambah Webhook Log',
        'subtitle' => 'Tambahkan data webhook log manual untuk testing atau audit.'
    ])

    @include('layouts.partials.flash-message')

    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('integrasi.webhook-log.store') }}" method="POST" class="grid grid-cols-1 gap-6 md:grid-cols-2">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="event_type" value="{{ old('event_type') }}"
                    placeholder="Contoh: attlog / get_userinfo / get_userid_list"
            ...
            <div>
                ...
                <input type="text" name="status" value="{{ old('status', 'received') }}"
                    placeholder="received / error / ignored"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Received At</label>
                <input type="datetime-local" name="received_at" value="{{ old('received_at') }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="message" rows="3"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">{{ old('message') }}</textarea>
            </div>
