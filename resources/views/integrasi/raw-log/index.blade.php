<x-app-layout>
    @include('layouts.partials.page-header', [
        'title' => 'Raw Log',
        'subtitle' =>
            'Kelola data scan mentah dari mesin Fingerspot untuk audit integrasi dan proses absensi harian.',
        'actions' =>
            '<a href="' .
            route('integrasi.raw-log.create') .
            '" class="inline-flex items-center rounded-2xl bg-white px-4 py-2 text-sm font-semibold text-blue-700 shadow-sm ring-1 ring-white/70 hover:bg-blue-50">+ Tambah Raw Log</a>',
    ])
    ...
    <div class="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="GET" action="{{ route('integrasi.raw-log.index') }}"
            ...
            <div>
                ...
                <input type="text" name="pin" value="{{ $pin }}" placeholder="Cari PIN"
                    class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div>
                ...
                <input type="text" name="verify_mode" value="{{ $verifyMode }}" placeholder="Contoh: 1 / 4"
                    class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Status Scan</label>
                <input type="text" name="status_scan" value="{{ $statusScan }}" placeholder="Contoh: 0"
                    class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div class="md:col-span-6 flex items-end gap-3">
                ...
                <a href="{{ route('integrasi.raw-log.index') }}"
                    class="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
                    Reset
                </a>
            </div>
        </form>
                        ...
                        @forelse ($items as $item)
                            <tr class="hover:bg-slate-50 align-top">
                                <td class="px-4 py-4 text-sm text-slate-700">
                                    {{ $item->scan_time ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $item->scan_time)->format('d-m-Y H:i:s') : '-' }}
                                </td>
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('integrasi.raw-log.destroy', $item) }}" method="POST"
                                            onsubmit="return confirm('Yakin ingin menghapus raw log ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
