<x-app-layout>
    @include('layouts.partials.page-header', [
        'title' => 'Edit Push Log',
        'subtitle' => 'Perbarui data push log untuk kebutuhan audit atau koreksi.'
    ])

    @include('layouts.partials.flash-message')

    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('integrasi.push-log.update', $pushLog) }}" method="POST" class="grid grid-cols-1 gap-6 md:grid-cols-2">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="device_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
                    ...
                    @foreach ($deviceOptions as $device)
                        <option value="{{ $device->id }}" @selected(old('device_id', $pushLog->device_id) == $device->id)>
                            {{ $device->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="pin" value="{{ old('pin', $pushLog->pin) }}"
            ...
            <div>
                ...
                <input type="text" name="action" value="{{ old('action', $pushLog->action) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>

            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Status</label>
                <input type="text" name="status" value="{{ old('status', $pushLog->status) }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="processed_at"
                    value="{{ old('processed_at', $pushLog->processed_at ? $pushLog->processed_at->format('Y-m-d\TH:i') : '') }}"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="response_message" rows="3"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">{{ old('response_message', $pushLog->response_message) }}</textarea>
            </div>

            <div class="md:col-span-2">
                <label class="mb-2 block text-sm font-semibold text-slate-700">Payload JSON</label>
                <textarea name="payload_json" rows="10"
                    class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100">{{ old('payload_json', $pushLog->payload_json) }}</textarea>
            </div>
